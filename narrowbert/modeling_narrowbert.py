...
class NarrowBertEncoder(nn.Module):
    def __init__(self, config, narrow=False):
        ...
        if not narrow:
            ...
            self.layer = nn.ModuleList([NarrowBertLayer(config) for _ in range(config.num_hidden_layers - config.full_length_layers)])
...
class NarrowBertLayer(nn.Module):
...
class NarrowBertModel(BertPreTrainedModel):
    """
    ...
    def __init__(self, config, add_pooling_layer=True, mode="mlm"):
    ...
    def forward(
        ...
        if self.mode == "mlm":
            ...
            for i in range(batch_size):
                batch_input = encoder_hidden_states[i][narrow_indicator[i]]
...
class NarrowBertForMaskedLM(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.bert = NarrowBertModel(config, add_pooling_layer=False)
    ...
    def forward(
        ...
        if labels is not None:
            narrow_indicator = labels > -100
            batch_size, _ = labels.shape
            batch_labels = []
            for i in range(batch_size):
                batch_label = labels[i][narrow_indicator[i]]
        ...
        outputs = self.bert(
            ...
            output_hidden_states=output_hidden_states,
            return_dict=return_dict,
            narrow_indicator=narrow_indicator,
        )
...
class NarrowBertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config

        self.bert = NarrowBertModel(config, mode="finetune")
        classifier_dropout = (
...
class NarrowBertForTokenClassification(BertPreTrainedModel):
